...
def simulate_conditional_gp(
    ...
    mean_vector = mean_fn(t) + build_kernel_matrix(t, t_obs,
                                                   kernel_fn) @ inverse_kernel @ (x_obs - mean_fn(t_obs))
    kernel_matrix = build_kernel_matrix(t, t, kernel_fn) - build_kernel_matrix(
        t, t_obs, kernel_fn) @ inverse_kernel @ build_kernel_matrix(t_obs, t, kernel_fn)

    # Finally the simulation, X, is obtained using 'multivariate_normal' from
    # numpy with method 'svd'
    X = np.random.default_rng().multivariate_normal(
        mean_vector, kernel_matrix, M, method='svd')
...
def gp_regression(
    ...
    # variance = k(X_test, X_test) - k(X_test, X) (k(X, X) + sigma^2 * I)^(-1)
    # k(X, X_test)
    prediction_mean = kernel_fn(X_test, X) @ inverse_matrix_y
    prediction_variance = kernel_fn(X_test,
                                    X_test) - kernel_fn(X_test,
                                                        X) @ inverse_matrix @ kernel_fn(X,
                                                                                        X_test)
